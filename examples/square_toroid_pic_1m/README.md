...
PIC out-of-plane = -lab y           (right-handed x,z,-y basis)
```
...
±lab-y, i.e. **out of the cross-section plane**. The signed storage component is
`external_bz = -B_y` (and therefore `-B_φ` on the positive-x meridian); it
magnetizes the particles and confines them within the cross-section. The current
sheets' poloidal field lies **in-plane** (`external_bx`, `external_by`).
...
absorbing particle BC is an inset proxy surface, 1.5 cm inside the ideal
filament boundary on each side, so the alive-count time series measures escape
toward the magnet wall without sampling its singular zero-radius filaments.
...
Equal densities and opposite equal-magnitude charges make the patch globally
quasineutral; independently sampled finite populations retain local particle
noise.
...
`μ₀ N I / (2π R₀) ≈ 0.040 T` because eight discrete rectangular coils
are not the continuous, uniformly wound toroid assumed by that estimate; their
ripple produces a substantial local enhancement. The axisymmetric azimuthal
current sheets contribute only the in-plane poloidal components, not `B_φ`.
The 1 eV μ⁻ thermal gyroradius is then **≈ 0.4 mm**, comfortably inside the
0.15 m block, so the species are well-confined.
...
# Full 5 µs run (at most ~2.0M CFL steps; the final step is clipped exactly)
PYTHONPATH=build/hip-gfx942-release/python \
  python -m quasar.pic.cli run examples/square_toroid_pic_1m/input.yaml
...
`plane: xz`, `external_bz` is the component along `-lab y` (`-B_φ` on the
positive-x meridian), while the in-plane axes (`external_bx`, `external_by`) are
lab x and lab z respectively.
