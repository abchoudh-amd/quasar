...
def build_pic_yaml() -> str:
    ...
    lines.extend([
        ...
        "  # are PEC: the first-order Mur 'outflow' BC is numerically unstable when",
        "  # coupled to the deposited particle current (field energy blows up",
        "  # exponentially within ~1000 steps regardless of current smoothing or",
        "  # density), whereas PEC is stable to the full run. Self-fields are tiny",
        "  # vs the static external quadrupole, so PEC reflection is harmless here.",
