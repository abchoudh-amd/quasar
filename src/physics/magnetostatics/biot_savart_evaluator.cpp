...
Field<Mat3x3> BiotSavartEvaluator::evaluate_grad_B(const ConductorSystem& cs,
  ...
  Field<Mat3x3> result(static_cast<std::size_t>(M));
  if (N == 0 || M == 0) {
    for (std::size_t i = 0; i < result.size(); ++i) {
      result[i] = Mat3x3{};
    }
  ...
  DeviceBuffer<Real> d_I (N);
  DeviceBuffer<Real> d_px(M), d_py(M), d_pz(M);
  // 9 components per observation point, laid out as (9, M) row-major.
  DeviceBuffer<Real> d_G(static_cast<std::size_t>(9) * static_cast<std::size_t>(M));
  ...
  std::vector<Real> hG(static_cast<std::size_t>(9)
  ...
  for (int i = 0; i < M; ++i) {
    ...
    Mat3x3 g;
    g.r0 = Vec3{hG[0 * MM + mi], hG[1 * MM + mi], hG[2 * MM + mi]};
    g.r1 = Vec3{hG[3 * MM + mi], hG[4 * MM + mi], hG[5 * MM + mi]};
    g.r2 = Vec3{hG[6 * MM + mi], hG[7 * MM + mi], hG[8 * MM + mi]};
    result[mi] = g;
  }
  return result;
}
