...
TEST(PicSingleParticle, CartesianGyroSignFrequencyAndSpeed) {
  ...
  const double theta = steps * 2.0 * std::atan(0.5 * b * dt);
...
TEST(PicSingleParticle, ExtremeElectricProductDoesNotUnderflowAtIntermediateStep) {
  ...
  // In binary64, (qm*dt) underflows to zero even though qm*dt*E is a normal,
  // representable kick. The exponent-scaled pusher must retain that product.
  const double expected = static_cast<double>(
      static_cast<long double>(qm) * static_cast<long double>(dt)
...
TEST(PicSingleParticle, CylindricalAxialFieldUsesRightHandedRphiZBasis) {
  ...
  const double theta = 2.0 * std::atan(0.5 * b_z * dt);
